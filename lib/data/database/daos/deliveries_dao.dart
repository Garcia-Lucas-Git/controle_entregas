...
class DeliveriesDao extends DatabaseAccessor<AppDatabase>
  ...
  Future<DeliveriesTableData?> getDeliveryById(int id) => (select(
    db.deliveriesTable,
  )..where((t) => t.id.equals(id))).getSingleOrNull();

  Stream<DeliveriesTableData?> watchDeliveryById(int id) => (select(
    db.deliveriesTable,
  )..where((t) => t.id.equals(id))).watchSingleOrNull();

  Future<int> countCompletedInRoute(int routeId) async {
    final rows =
        await (select(db.deliveriesTable)..where(
              (t) => t.routeId.equals(routeId) & t.status.equals('completed'),
            ))
            .get();
    return rows.length;
  }
  ...
  Future<int> setInProgress(int id) =>
      (update(db.deliveriesTable)..where((t) => t.id.equals(id))).write(
        const DeliveriesTableCompanion(status: Value('in_progress')),
      );
